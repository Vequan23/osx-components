...
withDefaults(defineProps<{
  ...
  disabled?: boolean;
  loading?: boolean;
}>(), { variant: "default", size: "medium", disabled: false, loading: false });
</script>
...
button { min-height: 30px; padding: 5px 15px; border: 1px solid var(--osx-border); border-radius: 7px; color: var(--osx-text); background: linear-gradient(var(--osx-surface-raised), var(--osx-surface-sunken)); box-shadow: 0 1px var(--osx-highlight) inset, 0 1px 2px rgba(22, 38, 48, .18); font: 600 13px/1.2 var(--osx-font); text-shadow: 0 1px var(--osx-highlight); cursor: pointer; }
...
.spinner { width: 10px; height: 10px; display: inline-block; margin-right: 6px; border: 2px solid currentColor; border-right-color: transparent; border-radius: 50%; animation: spin .7s linear infinite; vertical-align: -1px; }

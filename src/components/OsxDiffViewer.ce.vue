...
<template>
  <section :aria-label="label" class="viewer">
    ...
    <div v-else class="code split" role="table" aria-label="Split diff">
...
button { min-height: 25px; padding: 3px 8px; border: 1px solid var(--osx-border); border-radius: 6px; color: var(--osx-text); background: linear-gradient(var(--osx-surface-raised),var(--osx-surface-sunken)); font: 700 12px var(--osx-font); cursor: pointer; }button:focus-visible { outline: 3px solid var(--osx-focus); outline-offset: 1px; }
.code { max-height: 360px; overflow: auto; font: 12px/1.55 ui-monospace,SFMono-Regular,Consolas,monospace; }.line { min-width: max-content; display: grid; grid-template-columns: 42px 42px 24px minmax(400px,1fr); }.line > * { padding-block: 2px; }.number { padding-inline: 7px; color: var(--osx-muted); background: color-mix(in srgb,var(--osx-surface-sunken) 86%,transparent); text-align: right; user-select: none; }.marker { text-align: center; user-select: none; }.line code { padding-inline: 8px 18px; white-space: pre; }.line.add,.split code.add { background: color-mix(in srgb,var(--osx-success) 16%,var(--osx-surface)); }.line.remove,.split code.remove { background: color-mix(in srgb,var(--osx-danger) 15%,var(--osx-surface)); }.line.add .marker { color: var(--osx-success); }.line.remove .marker { color: var(--osx-danger); }.line.hunk { grid-template-columns: 42px 42px 24px minmax(400px,1fr); color: var(--osx-accent); background: color-mix(in srgb,var(--osx-accent) 10%,var(--osx-surface)); }

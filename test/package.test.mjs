...
test("ships Aqua, Graphite, Panther, and Dark Graphite token contracts", async () => {
  const theme = await read("src/theme.css");
  for (const themeName of ["aqua", "graphite", "panther", "graphite-dark"]) assert.match(theme, new RegExp(`data-osx-theme="${themeName}"`));
...
test("agent primitives expose bounded interactions without owning a provider", async () => {
  ...
  assert.match(composer, /input\.value = ""/);
  ...
  for (const name of ["OsxAgentComposerOption", "OsxAgentComposerSuggestion", "OsxAgentComposerSuggestionSelection", "OsxAgentComposerContextItem", "OsxAgentComposerAttachment", "OsxAgentComposerSubmission"]) assert.match(types, new RegExp(name));

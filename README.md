...
Agent events preserve backend neutrality. `<osx-agent-composer>` emits `input`, `submit`, `stop`, suggestion queries and selections, command selections, context and attachment changes, runtime selection changes, and optional voice requests. Set `allow-submit-while-running` when a host can queue or redirect follow-ups during a streaming run; Send remains available alongside Stop, while submitting and stopping states still block duplicate delivery. The paperclip opens a native multiple-file picker; selected, pasted, and dropped files emit through `attachment-add` for the host to validate and upload. `suggestion-select` and `command-select` include the selected item followed by resolved query, behavior, and composer-value metadata. The first `submit` argument remains the prompt string; the second contains the selected context, attachments, model, reasoning, and access IDs. `<osx-agent-approval>` emits `approve` and `reject`; `<osx-thinking>` emits `toggle`; `<osx-artifact>` emits `copy`, `download`, and `open`; `<osx-markdown>` emits `copy`; and citations coordinate through `activate` and `select`. `<osx-diff-viewer>` emits `view-change` and `copy`; `<osx-terminal>` emits `rerun`, `interrupt`, and `clear`; `<osx-file-tree>` emits `select` and `toggle`. `<osx-alert>` emits `dismiss`; `<osx-toast>` emits `dismiss` with either `manual` or `timeout` as its reason. Your application owns discovery, model calls, upload validation and transport, voice capture, tool execution, permission policy, persistence, and streaming transport.
